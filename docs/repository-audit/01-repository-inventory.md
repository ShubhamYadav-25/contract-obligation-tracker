...
| LLM               | `GROQ_API_KEY`, `GROQ_EXTRACTION_MODEL`                             | `GroqLlmAdapter` in worker runtime             | partially integrated | Groq adapter is selected for obligation extraction when configured.                                                            |
...
| Groq/OCR           | `GROQ_API_KEY`, `GROQ_EXTRACTION_MODEL`, `OCR_PROVIDER`, `TESSERACT_WORKER_COUNT`                                | partially integrated        |
...
| OCR/LLM/email adapters                      | partially integrated | Groq LLM selection is wired for obligation extraction; OCR/email adapters remain incomplete or provider-selected later.                                                                |
